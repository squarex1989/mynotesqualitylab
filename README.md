...
- **离线设备自动清理** —— 关了网页、离线超过 10 分钟（可用 `DEVICE_PRUNE_AFTER_MS` 调）的设备会被定期清掉（`DEVICE_PRUNE_INTERVAL_MS` 控制多久扫一次），分到它的角色变回未分配。断线重连这种几秒到几十秒的抖动不会被误清
- **转录对比** —— 顶部随时有「Compare」入口，谁都能看结果；只有房主能贴 My Notes / Granola / Otter 三家录出来的转录、改 glossary、发起打分。大部分指标（Plain/Weighted WER、UER、DER、专有名词、数字）是编辑距离算出来的，只有「关键内容有没有丢」「意思有没有被弄反」这两件交给模型判断
